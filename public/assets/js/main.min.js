        ...
        var e = {
            init: function () {
                ...
                Wolmart.call(Wolmart.ratingTooltip, 500),
                    ...
                    Wolmart.$body.on("click", "a[data-order-id]", function (t) {
                        console.log(t);
                        var popupHtml = `
                            <div style="padding:20px; max-width:100%;">
                                <h4 class="mb-2 text-danger fw-bold">Please pay to proceed this order</h4>
                                <form id="popupForm" enctype="multipart/form-data">
                                    <div class="mt-3 mb-2 p-3 border rounded bg-light">
                                        <div class="text-center mb-2">
                                            <img src="${baseSRC}/assets/images/payment/payment_qr.jpg"
                                                alt="Payment QR"
                                                style="max-width:180px; width:100%; border:1px solid #ccc; border-radius:8px; padding:5px;">
                                        </div>

                                        <h5 class="text-muted mb-3 mt-4" style="font-size:14px;">
                                            Scan the QR code using any UPI app. Once paid, fill the payment details below.
                                        </h5>
                                    </div>
                                    <div class="row">
                    ...
                    Wolmart.$body.on("click", ".product:not(.product-select) .btn-cart, .btn-push-to-shopify, .product-popup .btn-cart, .home .product-single .btn-cart", async function (e) {
                        e.preventDefault();
                        var i = t(this), a = i.closest(".product, .product-popup"),
                            d = new FormData();
                        d.append("product_id", i.data('productId'));

                        const request = {
                            productPrice: a.find(".product-price").text(),
                            productMedia: a.find(".product-media a img, .product-image > img")[0].src,
                            productName: a.find(".product-name, .product-title").text(),
                            headerText: "List to Shopify",

                        };
                        const userResponse = await ListOnShopifyResponse(request);
                        d.append("domain", userResponse?.selectDomain);
                        d.append("selling_price", userResponse?.sellingPrice);
                        if (!userResponse) {
                            return;
                        }
                        ...
                        try {
                            toggleLoader();
                            const res = await fetch(appendParams(`${window.baseSRC}/product/push-to-shopify`), {
                                method: "POST",
                                body: d,
                            ...
                            if (response.status) {
                                ...
                                throw new Error(response?.message || "Some thing went worng :(");
                            }
                    ...
                    Wolmart.$body.on("click", ".dropdown-box .logout", async function (e) {
                        e.preventDefault();
                        let closeFn;
                        console.log(e.currentTarget.dataset.id);
                        Wolmart.popup(
                            {
                                items: { src: t('#' + e.currentTarget.dataset.id)[0].innerHTML },
                                callbacks: {
                                    open: function () {
                                        console.log(this);
                                        this.contentContainer[0].style.maxWidth = '32rem';
                                        let thi = this;
                                        closeFn = () => this.close();
                                        this.content[0].querySelector('button.popupcancel').addEventListener("click", closeFn);
                                    },
                                    close: function () {
                                        this.bgOverlay[0].remove();
                                        this.content[0].querySelector('button.popupcancel').removeEventListener("click", closeFn);
                    ...
                    Wolmart.$body.on("click", ".btn-calculator", function (e) {
                        e.preventDefault();

                        Wolmart.popup(
                            {
                                items: { src: "#calculator-popup" },
                                type: "inline",
                                callbacks: {
                                    open: function () {
                                        let image = window.product.image.image_path.includes('http') ? window.product.image.image_path : `${baseSRC}/storage/${window.product.image.image_path}`;
                                        t("#calculator-popup").find("img").attr("src", image);
                                        t("#calculator-popup").find(".app-price").text(`₹ ${window.product.price}`);
                                        t("#calculator-popup").find("#appPrice").val(window.product.price);
                                        $("#calcBtn").on("click.calc", function () {

                                            // let $profitLoss = $('#profitLoss');
                                            // let $orderDiv = $('#orderDiv');
                                            // let $marginPerOrderDiv = $('#marginPerOrderDiv');
                                            // let $totalSpends = $('#totalSpends');
                                            // // Inputs
                                            // let appPrice = parseFloat($("#appPrice").val()) || 0;
                                            // let sellPrice = parseFloat($("#sellPrice").val()) || 0;
                                            // let rtoCharge = parseFloat($("#rtoCharges").val()) || 0;
                                            // let totalOrders = parseFloat($("#totalOrders").val()) || 0;
                                            // let confOrders = parseFloat($("#confOrders").val()) || 0;
                                            // let expDelivery = parseFloat($("#expDelivery").val()) || 0;
                                            // let adSpend = parseFloat($("#adSpend").val()) || 0;
                                            // let misc = parseFloat($("#miscCharges").val()) || 0;

                                            // // Formula (from Excel)
                                            // let profitPerOrder = sellPrice - appPrice;
                                            // let deliveredOrders = Math.round(confOrders * (expDelivery / 100));
                                            // let rtoOrders = confOrders - deliveredOrders;
                                            // let deliveredProfit = deliveredOrders * profitPerOrder;
                                            // let rtoChargesTotal = rtoOrders * rtoCharge;
                                            // let adSpendTotal = confOrders * adSpend;

                                            // // Final profit
                                            // let finalProfit = deliveredProfit - (rtoChargesTotal + adSpendTotal + misc);

                                            // // Update UI
                                            // var $span = $orderDiv.find("span");
                                            // $span[0].textContent = confOrders;
                                            // $span[1].textContent = deliveredOrders;
                                            // $span[2].textContent = rtoOrders;
                                            // $span[3].textContent = confOrders - deliveredOrders;

                                            // $span = $marginPerOrderDiv.find("span");
                                            // $span[0].textContent = sellPrice - appPrice;
                                            // $span[1].textContent = deliveredOrders;

                                            // $span = $totalSpends.find("span");
                                            // $span[0].textContent = adSpendTotal;
                                            // $span[1].textContent = rtoChargesTotal;
                                            // $span[2].textContent = misc;

                                            // //$("#netLoss").text("₹ " + finalProfit.toFixed(2));

                                            // // Optional: color based on profit/loss
                                            // if (finalProfit >= 0) {
                                            //     $profitLoss.removeClass("net-loss").addClass("net-profit");
                                            //     $profitLoss.find('#netLoss').addClass('text-success-bold');
                                            //      $profitLoss.find('#netLossPerOrder').addClass('text-success-bold');
                                            //     $profitLoss.find("p span").css("color","#05c605"); 
                                            // } else {
                                            //     $profitLoss.removeClass("net-profit").addClass("net-loss");
                                            //      $profitLoss.find('#netLoss').removeClass('text-success-bold');
                                            //      $profitLoss.find('#netLossPerOrder').removeClass('text-success-bold');
                                            //     $profitLoss.find("p span").css("color","");
                                            // }
                                            // console.log(finalProfit,"finalProfit....");
                                            // console.log($profitLoss.find("p span"));
                                            // $profitLoss.find("p span")[0].textContent = `₹  ${finalProfit.toFixed(2)}`;
                                            // $profitLoss.find("p span")[1].textContent = `₹  ${(finalProfit / (deliveredOrders/100)).toFixed(2)}`;
                                            let $profitLoss = $('#profitLoss');
                                            let $orderDiv = $('#orderDiv');
                                            let $marginPerOrderDiv = $('#marginPerOrderDiv');
                                            let $totalSpends = $('#totalSpends');
                                            ...
                                            let sellPrice = parseFloat($("#sellPrice").val()) || 0;
                                            let rtoCharge = parseFloat($("#rtoCharges").val()) || 0;
                                            let totalOrders = parseFloat($("#expOrders").val()) || 0;
                                            let confOrders = parseFloat($("#confOrders").val()) || 0;
                                            let expDelivery = parseFloat($("#expDelivery").val()) || 0;
                                            let adSpend = parseFloat($("#adSpend").val()) || 0;
                                            let misc = parseFloat($("#miscCharges").val()) || 0;
                                            ...
                                            let totalEarnings = profitPerOrder * deliveredOrders;

                                            let adSpendTotal = totalOrders * adSpend;  // FIXED
                                            let rtoChargesTotal = rtoOrders * rtoCharge;

                                            let totalSpends = adSpendTotal + misc + rtoChargesTotal;

                                            let finalProfit = totalEarnings - totalSpends;
                                            ...
                                            $span = $marginPerOrderDiv.find("span");
                                            $span[0].textContent = profitPerOrder;
                                            $span[1].textContent = deliveredOrders;
                                            ...
                                            // FIXED: Correct profit per order
                                            let profitPerOrderFinal = finalProfit / totalOrders;
                                            $profitLoss.find("p span")[1].textContent = `₹ ${profitPerOrderFinal.toFixed(2)}`;
                                            console.log((finalProfit / (deliveredOrders / 100), '------------ profit per order'));
                                        });
                                    },
                                    close: function () {
                                        $("#calcBtn").off(".calc");
